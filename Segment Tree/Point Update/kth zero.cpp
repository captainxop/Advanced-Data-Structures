...
// 2: find the number of zeros in a given range, and additionally find the index of the k-th zero.
...
int get_kth(int node, int L, int R, int k)
{
	...
	int M = (L + R) / 2;
	if (st[node * 2] >= k) {
		return get_kth(node * 2, L, M, k);
	}
	else {
		return get_kth(node * 2 + 1, M + 1, R, k - st[node * 2]);
	}
}
...
void solve()
{
	...
	while (q--) {
		...
		if (t == 1) {
			...
			count = query(1, 0, n - 1, l, r);
			idx = get_kth(1, 0, n - 1, k);
			cout << count << ' ' << idx << '\n';
		}
	}
}
